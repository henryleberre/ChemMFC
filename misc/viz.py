...
s=10*(2*D*te)**0.5
# Have at least 20 cells across the mixing layer.
N=20*int(s/((2*D*ts)**0.5))
v=0
dt=1e-7
NT=int((te-ts)/dt)

def a_ref(x, t_step):
    t = t_step / (NT) * (te - ts) + ts
    erfp = 1 - math.erf(x / (2.0 * math.sqrt(D * t)))
    #erfp = math.erf((x + a) / (2.0 * math.sqrt(D * t)))
    #erfm = math.erf((x - a) / (2.0 * math.sqrt(D * t)))
    #a1 = pb + (pa - pb) / 2.0 * (erfp - erfm)
    a1 = pb + (pa - pb) / 2.0 * erfp
    return a1

def y1_ref(x, t_step):
    a1 = a_ref(x, t_step)
    y1 = pa * (pb - a1) / (a1 * (pb - pa))
    return 1 - y1

def y2_ref(x, t_step):
    return 1 - y1_ref(x, t_step)

def c1_ref(x, t_step):
    return y1_ref(x, t_step) * a_ref(x, t_step)

def c2_ref(x, t_step):
    return y2_ref(x, t_step) * a_ref(x, t_step)

def a_ref2(x, t_step):
    return c1_ref(x, t_step) + c2_ref(x, t_step)
...
PLOTS = {
    ...
    #'Species (cons)': [('$\\rho Y_\text{H2}$',         'cons.5', '#FFFFFF'),
    #                   ('$\\rho Y_\text{O2}$',         'cons.8', '#FFFFFF'),
    #                   ('$\\rho Y_\text{H20}$',        'cons.10', '#FFFFFF'),
    #                   ('$\\rho Y_\text{N2}$',         'cons.14', '#FF00FF')]
}
...
def load_var_data(var):
    ...
    #reqts = ARGS.get('num_frames', len(dfs))

    # Filter frames to capture (if -n is specified)
    #for k in sorted(dfs.keys())[nreqts:]:
    #    del dfs[k]

    #if len(dfs) == 0 or len(dfs) < nreqts:
    #    print(f" * Error: Not enough frames to process.")
    #    print(f" * Found {len(dfs)} frames, requested {nreqts}.")
    #    exit(1)
